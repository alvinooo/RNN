...
class RNN:
    ...
    def __init__(sf):
        ...
        sf.datasetLen = 0 # Defined in builInputData

        sf.buildInputData("trainData3.txt")
        sf.buildLabelData("trainData3.txt")
    ...
    def buildInputData_sliding_window(sf, filename):
        f = open(filename, 'r')
        data = f.read()
    ...
    def buildInputData(sf, filename):
        f = open(filename, 'r')
    ...
    def buildLabelData(sf, filename):
        f = open(filename, 'r')
    ...
    def generate(sf):
        ...
        print "*** testing ***"
        #asciiChar = sf.allData[0][0]
        str1 = ""
        #str1 += asciiChar

        sf.buildInputData("validate.txt")
        sf.buildLabelData("validate.txt")
        
        sf.setHiddenActivation()
        asciiChar = sf.allData[0][0]
        str1 += asciiChar
        for t in range(sf.sequenceLen):
            asciiChar = sf.allData[0][t]
            sf.calc_y_hidden_layer(asciiChar, t, calc_type)
            asciiChar = sf.calc_y_softmax_output_layer(0,t,calc_type)
            str1 += asciiChar
        pos = 1
        for seq in range(1,sf.datasetLen):
            if pos == sf.sequenceLen:
                pos = 0
                seq += 1
            if seq == sf.datasetLen:
                break
            asciiChar = sf.allData[seq][pos]
            str1 += asciiChar
    ...
    def forward_back_prop_single_epoch(sf): 
        ...
        print "Training Accuracy for ", sf.datasetLen, "data examples: ", acc
